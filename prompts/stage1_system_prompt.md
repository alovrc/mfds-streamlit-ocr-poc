...
- 제품 마스터에서 확정되지 않았더라도 `food_confidence >= 0.80`이고 `food_confidence > hff_confidence`이며 상충 근거가 없으면 `FOOD`로 판정한다. DB 미일치 등 불확실성 코드와 `requires_human_review=true`는 유지한다.
- 위 고신뢰 식품 조건을 충족하지 못하고 제품 마스터에도 없으면 `FOOD_FALLBACK`으로 처리한다. `FOOD_FALLBACK`의 `food_confidence`는 0.80 미만이어야 한다.
- `FOOD_FALLBACK`은 일반식품 확정값이 아니다.
...
- 식품 또는 FOOD_FALLBACK: `FOOD_REVIEW` / `FS11_FOOD_REVIEW`
- 분석대상 외: `NO_STAGE2` / `FS01_PRODUCT_GATE`
