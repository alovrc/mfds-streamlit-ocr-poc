...
- 입력 payload의 `stage1_uncertainty_codes`에 `INPUT_INCOMPLETE`가 있으면
  ...
  `INSUFFICIENT_EVIDENCE` 및 `INPUT_INCOMPLETE`로 담당자 검토를 요구한다.
...
`케어` 같은 단독 표현만으로 탐지하지 않는다. 일반 건강·영양 정보 표현은
`GENERAL_HEALTH`로 분류하고 부당광고 후보에 연결하지 않는다. 제품 연결,
원문에 실제 존재하는 인용문, 구체적인 금지 유형의 직접 근거가 모두 있을
때만 `PROHIBITED_CANDIDATE`로 분류한다. 하나의 표현에 여러 법령 호를
연결하려면 각 호별 독립적인 직접 근거가 있어야 하며, 그렇지 않으면 가장
구체적인 한 호만 남기고 나머지는 반환하지 않는다. 제품유형 불확실성이나
검색자료 부족 자체를 고점으로 바꾸지 않는다. 공식 근거가 검색되지
않더라도 문제표현 후보는 유지하고 `SEARCH_NO_OFFICIAL_EVIDENCE`를
표시한다. 이 경우 법령 호는 후보로만 표시하고 최종 상태는 `REVIEW`로
유지한다. 앱이 원문 문제표현과 로컬 Rule ID를 연결하여 최종 유효성을
결정한다.
...
표시한다. 공식근거가 없는 양성 후보는 자동 부적합으로 확정하지 않고
`REVIEW`로 표시한다. `POSSIBLE` 판매성향도 자동 부적합이 아니라
`REVIEW`로 표시한다. 공식근거와 적발사례 ID가 겹치면 해당 제품 결과만
`EVIDENCE_CASE_TYPE_MISMATCH`와 함께 격리하며 전체 배치를 실패시키지 않는다.
