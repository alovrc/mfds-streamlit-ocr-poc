...
flowchart TD
    ...
    G -->|불명확| J["2단계 FS11 계속·유형 불확실 REVIEW"]
...
| 두 점수가 모두 `0.50` 미만 | 품목 불명확, FS11 2단계 계속·REVIEW |
| 두 점수 차이가 `0.05` 이하 | 품목 불명확, FS11 2단계 계속·REVIEW |
| 제품유형 근거가 상충 | 품목 불명확, FS11 2단계 계속·REVIEW |
...
활성 위반 후보는 다음 조건을 모두 충족하면 유효하다.

1. 입력 원문에서 그대로 확인되는 제품 연결 문제표현이 있다.
2. 해당 위반유형에 대응하는 활성 로컬 Rule ID가 연결된다.
3. 구체적인 금지 유형에 대한 직접 근거가 있다.

일반 건강·영양 정보 표현은 `GENERAL_HEALTH`로 별도 보존하며
`PROBLEM_CANDIDATE`에 포함하지 않는다. 공식근거가 없으면 법령 호 후보만
표시하고 최종 상태는 `REVIEW`로 유지한다.
...
근거이다. 공식근거가 없는 양성 후보는 법령 호 후보만 표시하고 최종 상태를
`REVIEW`로 유지한다. `SEARCH_NO_OFFICIAL_EVIDENCE`는 담당자 검토 사유로
보존한다.
...
| --- | ---: |
| 제품유형 불명확으로 FS11 2단계 계속·REVIEW | 2회 |
